...
def preprocess_disease_data(df):
    ...
    t_events = df["t_hours"].values.astype(np.float32)
    lat_events = df["latitude"].values.astype(np.float32)
    lon_events = df["longitude"].values.astype(np.float32)
...
class HotspotPredictor:
    ...
    def predict_temporal_evolution(self, pincode, hours_ahead=72,
                                  step_hours=1.0):
        df = self.data["df_sorted"]
        t_events = self.data["t_events"]
        lat_events = self.data["lat_events"]
        ...
        loc = df[df["pincode"] == pincode].iloc[0]
